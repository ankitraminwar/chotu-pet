 ...
 */
import { createReadStream, createWriteStream, mkdirSync, readdirSync, statSync, readFileSync } from "fs";
...
for (const filepath of files) {
  ...
  entries.push({ arcname, nameBytes, crc, compressed, uncompressedSize, offset });
}
...
for (const e of entries) {
  centralChunks.push(Buffer.concat([
    ...
    uint16LE(0), uint16LE(0),              // disk start / internal attr
...
console.log(`✓ releases/chotu-pet-v${version}.zip (${kb} KB, ${entries.length} files)`);
